    ...
    return (
        <Tabs defaultValue="monthly" className="w-full flex flex-col items-center justify-center">
            ...
            {(["monthly", "yearly"] as Tab[]).map((billingCycle) => (
                <TabsContent key={billingCycle} value={billingCycle} className="grid grid-cols-1 lg:grid-cols-3 gap-5 w-full md:gap-8 flex-wrap max-w-5xl mx-auto pt-6">
                    {PLANS.map((plan) => (
                        <Card
                            key={plan.name}
                            className={cn(
                        ...
                        >
                            <CardHeader className={cn(
                                ...
                                <h5 className="text-3xl font-semibold flex items-end">
                                    ...
                                    {plan.name !== "Free" && (
                                        <span className="text-base text-muted-foreground font-normal">/{billingCycle}</span>
                                    )}
